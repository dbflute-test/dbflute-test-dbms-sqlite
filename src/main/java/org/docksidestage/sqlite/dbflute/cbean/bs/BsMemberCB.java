package org.docksidestage.sqlite.dbflute.cbean.bs;
...
public class BsMemberCB extends AbstractConditionBean {
    ...
    public BsMemberCB() {
        ...
        if (MaDBFluteConfig.getInstance().isSpecifyColumnRequiredWarningOnly()) {
    ...
    public static class HpSpecification extends HpAbstractSpecification<MemberCQ> {
         ...
         * @return The object to set up a function for referrer table. (NotNull)
         */
        public HpSDRFunction<MemberServiceCB, MemberCQ> derivedMemberService() {
            assertDerived("memberServiceList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<MemberServiceCB> sq, MemberCQ cq, String al, DerivedReferrerOption op)
