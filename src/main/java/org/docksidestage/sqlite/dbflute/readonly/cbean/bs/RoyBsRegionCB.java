 ...
 */
public class RoyBsRegionCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected RoyRegionCQ _conditionQuery;

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public RoyBsRegionCB() {
     ...
     * @return this. (NotNull)
     */
    public RoyRegionCB acceptPK(Integer regionId) {
        assertObjectNotNull("regionId", regionId);
        RoyBsRegionCB cb = this;
        cb.query().setRegionId_Equal(regionId);
        return (RoyRegionCB)this;
    }
     ...
     */
    public RoyRegionCQ query() {
        assertQueryPurpose(); // assert only when user-public query 
        return doGetConditionQuery();
    }

    public RoyRegionCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected RoyRegionCQ doGetConditionQuery() {
    ...
    protected RoyRegionCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected RoyRegionCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        RoyRegionCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected RoyRegionCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new RoyRegionCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<RoyRegionCB> unionCBLambda) {
        final RoyRegionCB cb = new RoyRegionCB(); cb.xsetupForUnion(this); xsyncUQ(cb); 
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final RoyRegionCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<RoyRegionCB> unionCBLambda) {
        final RoyRegionCB cb = new RoyRegionCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final RoyRegionCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
    ...
    public static class HpSpecification extends HpAbstractSpecification<RoyRegionCQ> {
        public HpSpecification(ConditionBean baseCB, HpSpQyCall<RoyRegionCQ> qyCall
         ...
         */
        public HpSDRFunction<RoyMemberAddressCB, RoyRegionCQ> derivedMemberAddress() {
            assertDerived("memberAddressList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<RoyMemberAddressCB> sq, RoyRegionCQ cq, String al, DerivedReferrerOption op)
         ...
         */
        public HpSDRFunction<RoyRegionCB, RoyRegionCQ> myselfDerived() {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<RoyRegionCB> sq, RoyRegionCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public RoyRegionCB dreamCruiseCB() {
        RoyRegionCB cb = new RoyRegionCB();
        cb.xsetupForDreamCruise((RoyRegionCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<RoyRegionCB> columnQuery(final SpecifyQuery<RoyRegionCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected RoyRegionCB xcreateColumnQueryCB() {
        RoyRegionCB cb = new RoyRegionCB();
        cb.xsetupForColumnQuery((RoyRegionCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<RoyRegionCB> orCBLambda) {
        xorSQ((RoyRegionCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<RoyRegionCB> andCBLambda) {
        xorSQAP((RoyRegionCB)this, andCBLambda);
    }
    ...
    protected void xprepareSyncQyCall(ConditionBean mainCB) {
        final RoyRegionCB cb;
        if (mainCB != null) {
            cb = (RoyRegionCB)mainCB;
        } else {
            cb = new RoyRegionCB();
        }
    ...
    protected String xgetConditionBeanClassNameInternally() { return RoyRegionCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return RoyRegionCQ.class.getName(); }
