 ...
 */
public class RoyBsProductCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected RoyProductCQ _conditionQuery;

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public RoyBsProductCB() {
     ...
     * @return this. (NotNull)
     */
    public RoyProductCB acceptPK(Integer productId) {
        assertObjectNotNull("productId", productId);
        RoyBsProductCB cb = this;
        cb.query().setProductId_Equal(productId);
        return (RoyProductCB)this;
    }
     ...
     */
    public RoyProductCQ query() {
        assertQueryPurpose(); // assert only when user-public query 
        return doGetConditionQuery();
    }

    public RoyProductCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected RoyProductCQ doGetConditionQuery() {
    ...
    protected RoyProductCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected RoyProductCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        RoyProductCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected RoyProductCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new RoyProductCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<RoyProductCB> unionCBLambda) {
        final RoyProductCB cb = new RoyProductCB(); cb.xsetupForUnion(this); xsyncUQ(cb); 
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final RoyProductCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<RoyProductCB> unionCBLambda) {
        final RoyProductCB cb = new RoyProductCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final RoyProductCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
    ...
    public static class HpSpecification extends HpAbstractSpecification<RoyProductCQ> {
        protected RoyProductStatusCB.HpSpecification _productStatus;
        public HpSpecification(ConditionBean baseCB, HpSpQyCall<RoyProductCQ> qyCall
         ...
         */
        public HpSDRFunction<RoyPurchaseCB, RoyProductCQ> derivedPurchase() {
            assertDerived("purchaseList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<RoyPurchaseCB> sq, RoyProductCQ cq, String al, DerivedReferrerOption op)
         ...
         */
        public HpSDRFunction<RoyProductCB, RoyProductCQ> myselfDerived() {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<RoyProductCB> sq, RoyProductCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public RoyProductCB dreamCruiseCB() {
        RoyProductCB cb = new RoyProductCB();
        cb.xsetupForDreamCruise((RoyProductCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<RoyProductCB> columnQuery(final SpecifyQuery<RoyProductCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected RoyProductCB xcreateColumnQueryCB() {
        RoyProductCB cb = new RoyProductCB();
        cb.xsetupForColumnQuery((RoyProductCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<RoyProductCB> orCBLambda) {
        xorSQ((RoyProductCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<RoyProductCB> andCBLambda) {
        xorSQAP((RoyProductCB)this, andCBLambda);
    }
    ...
    protected void xprepareSyncQyCall(ConditionBean mainCB) {
        final RoyProductCB cb;
        if (mainCB != null) {
            cb = (RoyProductCB)mainCB;
        } else {
            cb = new RoyProductCB();
        }
    ...
    protected String xgetConditionBeanClassNameInternally() { return RoyProductCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return RoyProductCQ.class.getName(); }
