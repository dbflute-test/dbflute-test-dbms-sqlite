...
public abstract class BsMemberBhv extends AbstractBehaviorWritable<Member, MemberCB> {
    ...
    public static final String PATH_various_wrongexample_selectEndCommentNotFound = "various:wrongexample:selectEndCommentNotFound";
     ...
     * @return The callback interface which you can load nested referrer by calling withNestedReferrer(). (NotNull)
     */
    public NestedReferrerListGateway<MemberService> loadMemberService(List<Member> memberList, ReferrerConditionSetupper<MemberServiceCB> refCBLambda) {
        xassLRArg(memberList, refCBLambda);
        return doLoadMemberService(memberList, new LoadReferrerOption<MemberServiceCB, MemberService>().xinit(refCBLambda));
    }
     ...
     * @return The callback interface which you can load nested referrer by calling withNestedReferrer(). (NotNull)
     */
    public NestedReferrerListGateway<MemberService> loadMemberService(Member member, ReferrerConditionSetupper<MemberServiceCB> refCBLambda) {
        xassLRArg(member, refCBLambda);
        return doLoadMemberService(xnewLRLs(member), new LoadReferrerOption<MemberServiceCB, MemberService>().xinit(refCBLambda));
    }

    protected NestedReferrerListGateway<MemberService> doLoadMemberService(List<Member> memberList, LoadReferrerOption<MemberServiceCB, MemberService> option) {
    ...
    public void insert(Member member) {
        InsertOption<MemberCB> op = new InsertOption<MemberCB>();
