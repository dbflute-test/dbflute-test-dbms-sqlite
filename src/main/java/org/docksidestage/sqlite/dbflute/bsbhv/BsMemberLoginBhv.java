...
public abstract class BsMemberLoginBhv extends AbstractBehaviorWritable<MemberLogin, MemberLoginCB> {
     ...
     * @throws SelectEntityConditionNotFoundException When the condition for selecting an entity is not found.
     */
    public OptionalEntity<MemberLogin> selectByPK(Integer memberLoginId) {
        return facadeSelectByPK(memberLoginId);
    }

    protected OptionalEntity<MemberLogin> facadeSelectByPK(Integer memberLoginId) {
        return doSelectOptionalByPK(memberLoginId, typeOfSelectedEntity());
    }

    protected <ENTITY extends MemberLogin> ENTITY doSelectByPK(Integer memberLoginId, Class<? extends ENTITY> tp) {
        return doSelectEntity(xprepareCBAsPK(memberLoginId), tp);
    }

    protected <ENTITY extends MemberLogin> OptionalEntity<ENTITY> doSelectOptionalByPK(Integer memberLoginId, Class<? extends ENTITY> tp) {
    ...
    public void insert(MemberLogin memberLogin) {
        InsertOption<MemberLoginCB> op = new InsertOption<MemberLoginCB>();
        op.xtoBeCompatibleInsertColumnNotNullOnly();
        doInsert(memberLogin, op);
    }
